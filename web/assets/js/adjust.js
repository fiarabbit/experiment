...
(function () {
    ...
    let sender = (/** @lends sender */function () {
        ...
        let reference = {
             ...
             */
                ...
                $.ajax({
                    url: "http://experiment.va/adjust/sendData",
                    type: "GET",
                    data: _dataObj
                })
            ...
            timer: {
                set: function (display, sender, data, adjuster) {
                    ...
                    if (!ended) {
                        ...
                        $(document).on('adjustEnd', function (ev, finalInterval) {
                            if (!ended) {
                                ended = true;
    ...
    let adjuster = (function (data) {
        let dataObj=data;
        const StepStepSize = 0.5;//good
        let stepSize = 0.5;//good
        ...
        let reference = {
            ...
            step: function (string, value) {
                if (!ended) {
                    ...
                    if (stepCounter > stepCounterThreshold) {
                        $(document).trigger('adjustEnd', nextValue);
                        alert("end");
                        window.location.href = 'http://experiment.va/adjust/finish/?hash='+dataObj.hash+'&timeLimit='+Math.round(dataObj.timeLimit)+'&username='+dataObj.username;
                        ended = true;
                    }
                    return nextValue;
                } else {
